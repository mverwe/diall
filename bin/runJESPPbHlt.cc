#include "UserCode/diall/analyzeJESPPbHLT.C"
...
int main(int argc, char* argv[])
{
  ...
  int doCaloJetJEC = 0;
  if(argc>4)
    doCaloJetJEC = atoi(argv[5]);
  Printf("doCaloJetJEC: %d",doCaloJetJEC);
  ...
  std::string outname = "AnaResultsJESHLT.root";
  int maxEvts = runProcess.getParameter<int>("maxEvents");
    
  analyzeJES(urls,outname.c_str(),maxEvts,firstFile,lastFile,firstEvent, doCaloJetJEC);
